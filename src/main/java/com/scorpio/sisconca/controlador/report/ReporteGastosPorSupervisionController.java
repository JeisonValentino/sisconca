...
@ManagedBean
@SessionScoped
public class ReporteGastosPorSupervisionController implements Serializable {
    ...
    public void contar() throws Exception {
        ...
        if (idArea != null) {
            param.put("idArea", idArea);
            sql = "select count(g) from ViewResumenGastos g where g.fecha>=:init and g.fecha<=:end "
                    + "and idArea=:idArea and nombre like :concepto";
        } else {
            sql = "select count(g) from ViewResumenGastos g where g.fecha>=:init and g.fecha<=:end "
                    + "and nombre like :concepto";
